...
def process_hotpot(dataset: Dataset) -> list:
    ...
    print(f"bridge %: {sum(q['type'] == 'bridge' for q in subset) / N_SAMPLES * 100:.1f}%")
    print(f"comparison %: {sum(q['type'] == 'comparison' for q in subset) / N_SAMPLES * 100:.1f}%")
...
def process_boolq(dataset: Dataset) -> list:
    ...
    print(f"False ratio: {(N_SAMPLES - true_count) / N_SAMPLES * 100:.1f}% ({N_SAMPLES - true_count})")

    return subset

def main():
    if "hotpot" in CONFIG.dataset_name:
        dataset = load_dataset(CONFIG.dataset_name, CONFIG.config, split=CONFIG.split, trust_remote_code=True)
